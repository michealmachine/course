...
import { DateLearningHeatmapVO, LearningHeatmapVO, LearningProgressTrendVO } from '@/types/learning-stats';
...
const institutionLearningStatsService = {
   ...
   * @returns 用户课程学习详情
   */
  getUserCourseLearningDetail: async (
    courseId: string | number,
    userId: string | number
  ): Promise<any> => {
    try {
      console.log(`开始获取用户课程学习详情, 课程ID: ${courseId}, 用户ID: ${userId}`);
      const response: AxiosResponse<ApiResponse<any>> =
        await request.get(`/institutions/learning-statistics/courses/${courseId}/users/${userId}/detail`);

      console.log('获取用户课程学习详情成功:', response.data);
      return response.data.data;
    } catch (error) {
      console.error(`获取用户课程学习详情失败, 课程ID: ${courseId}, 用户ID: ${userId}:`, error);
      throw error;
    }
  },
   ...
   * @returns 活动类型统计列表
   */
  getUserCourseActivityTypeStats: async (
    courseId: string | number,
    userId: string | number
  ): Promise<ActivityTypeStatVO[]> => {
    try {
      console.log(`开始获取用户课程活动类型统计, 课程ID: ${courseId}, 用户ID: ${userId}`);
      const response: AxiosResponse<ApiResponse<ActivityTypeStatVO[]>> =
        await request.get(`/institutions/learning-statistics/courses/${courseId}/users/${userId}/activity-types`);

      console.log('获取用户课程活动类型统计成功:', response.data);
      return response.data.data;
    } catch (error) {
      console.error(`获取用户课程活动类型统计失败, 课程ID: ${courseId}, 用户ID: ${userId}:`, error);
      throw error;
    }
  },
   ...
   * @returns 学习热力图数据
   */
  getUserCourseLearningHeatmap: async (
    courseId: string | number,
    userId: string | number,
    startDate?: string,
    endDate?: string
    ...
    try {
      console.log(`开始获取用户课程学习热力图, 课程ID: ${courseId}, 用户ID: ${userId}, 开始日期: ${startDate}, 结束日期: ${endDate}`);
      const response: AxiosResponse<ApiResponse<LearningHeatmapVO>> =
        await request.get(`/institutions/learning-statistics/courses/${courseId}/users/${userId}/heatmap`, {
          params: { startDate, endDate }
        });

      console.log('获取用户课程学习热力图成功:', response.data);
      return response.data.data;
    } catch (error) {
      console.error(`获取用户课程学习热力图失败, 课程ID: ${courseId}, 用户ID: ${userId}:`, error);
      throw error;
    }
  },
   ...
   * @param startDate 开始日期（YYYY-MM-DD格式）
   * @param endDate 结束日期（YYYY-MM-DD格式）
   * @returns 按日期分组的学习热力图数据
   */
  getUserCourseLearningHeatmapByDate: async (
    courseId: string | number,
    userId: string | number,
    startDate?: string,
    endDate?: string
    ...
    try {
      console.log(`开始获取用户课程学习热力图(按日期), 课程ID: ${courseId}, 用户ID: ${userId}, 开始日期: ${startDate}, 结束日期: ${endDate}`);
      const response: AxiosResponse<ApiResponse<DateLearningHeatmapVO>> =
        await request.get(`/institutions/learning-statistics/courses/${courseId}/users/${userId}/heatmap-by-date`, {
          params: { startDate, endDate }
        });

      console.log('获取用户课程学习热力图(按日期)成功:', response.data);
      return response.data.data;
    } catch (error) {
      console.error(`获取用户课程学习热力图(按日期)失败, 课程ID: ${courseId}, 用户ID: ${userId}:`, error);
      // 如果失败，返回空数据
      return {
   ...
   * @returns 学习进度趋势数据
   */
  getUserCourseLearningProgressTrend: async (
    courseId: string | number,
    userId: string | number,
    startDate?: string,
    endDate?: string
    ...
    try {
      console.log(`开始获取用户课程学习进度趋势, 课程ID: ${courseId}, 用户ID: ${userId}, 开始日期: ${startDate}, 结束日期: ${endDate}`);
      const response: AxiosResponse<ApiResponse<LearningProgressTrendVO>> =
        await request.get(`/institutions/learning-statistics/courses/${courseId}/users/${userId}/progress-trend`, {
          params: { startDate, endDate }
        });

      console.log('获取用户课程学习进度趋势成功:', response.data);
      return response.data.data;
    } catch (error) {
      console.error(`获取用户课程学习进度趋势失败, 课程ID: ${courseId}, 用户ID: ${userId}:`, error);
      throw error;
    }
  }
};
