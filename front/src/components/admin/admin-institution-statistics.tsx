...
interface AdminInstitutionStatisticsProps {
  institutionId: number;
  stats: any;
}

export function AdminInstitutionStatistics({ institutionId, stats }: AdminInstitutionStatisticsProps) {
  ...
  const fetchLearningStats = async () => {
    ...
    try {
      ...
      console.error('管理员组件 - 获取学习统计数据出错:', error);
  ...
  const fetchIncomeStats = async () => {
    ...
    try {
      ...
      // 获取机构收入趋势
      const incomeTrend = await adminInstitutionStatsService.getIncomeTrend(institutionId, '30d', 'day');
      ...
      console.error('管理员组件 - 获取收入统计数据出错:', error);
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats ? stats.userCount.toLocaleString() : <Skeleton className="h-8 w-20" />}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats ? formatDuration(stats.totalLearningDuration) : <Skeleton className="h-8 w-20" />}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats ? formatMoney(stats.totalIncome) : <Skeleton className="h-8 w-20" />}
            </div>
            {stats && (
              <p className="text-xs text-muted-foreground mt-1">
