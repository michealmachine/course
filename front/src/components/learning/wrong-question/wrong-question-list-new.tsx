...
export default function WrongQuestionListNew({
  courseId,
  ...
  const loadWrongQuestions = async () => {
    ...
    try {
      ...
      // 输出后端返回的数据，查看VO结构
      console.log('后端返回的错题数据:', result);
      if (result.content && result.content.length > 0) {
        console.log('第一条错题详情:', result.content[0]);
        console.log('正确答案字段:', result.content[0].correctAnswers);
        console.log('用户答案字段:', result.content[0].userAnswers);
      }
  ...
  const handleViewQuestion = async (wrongQuestion: UserWrongQuestionVO) => {
    ...
    try {
      ...
      // 输出当前错题对象，查看是否包含正确答案
      console.log('展开的错题对象:', wrongQuestion);
      console.log('错题中的正确答案:', wrongQuestion.correctAnswers);
      console.log('错题中的用户答案:', wrongQuestion.userAnswers);

      const questionDetailObj = {
        id: questionId,
      ...
      console.log('设置到state中的题目详情:', questionDetailObj);

      setQuestionDetails(prev => ({
        ...prev,
        [questionId]: questionDetailObj
      }));
                          ...
                          <TableRow>
                            <TableCell colSpan={showCourseColumn ? 6 : 5} className="p-0">
                              <div className="bg-muted/50 p-4 border-t border-b">
                                {questionDetails[question.questionId] && (
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                      <div>
                                        ...
                                        <div className="mt-1 p-3 bg-green-50 dark:bg-green-950/20 rounded-md border border-green-200 dark:border-green-800">
                                          {(() => {
                                            // 输出渲染时的正确答案数据
                                            console.log('渲染正确答案时的数据:', questionDetails[question.questionId].correctAnswers);
                                            return questionDetails[question.questionId].correctAnswers &&
                                              ...
                                              <ul className="list-disc list-inside">
                                                {questionDetails[question.questionId].correctAnswers.map((answer: string, index: number) => {
                                                  console.log(`正确答案项 ${index}:`, answer);
                                                  return (
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                      <div>
                                        ...
                                        <div className="mt-1 p-3 bg-green-50 dark:bg-green-950/20 rounded-md border border-green-200 dark:border-green-800">
                                          {(() => {
                                            // 输出渲染时的正确答案数据
                                            console.log('未解决标签页 - 渲染正确答案时的数据:', questionDetails[question.questionId].correctAnswers);
                                            return questionDetails[question.questionId].correctAnswers &&
                                                   questionDetails[question.questionId].correctAnswers.length > 0 ? (
                                              <ul className="list-disc list-inside">
                                                {questionDetails[question.questionId].correctAnswers.map((answer: string, index: number) => {
                                                  console.log(`未解决标签页 - 正确答案项 ${index}:`, answer);
                                                  return (
                                                    <li key={index}>{answer}</li>
                                                  );
                                                })}
