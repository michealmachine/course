...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Transactional
    public boolean resetPassword(String email, String emailCode) {
        log.info("开始重置密码: email={}", email);

        // 验证邮箱验证码
        boolean isValid = emailService.validateVerificationCode(email, emailCode);
        if (!isValid) {
        ...
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> {
                    log.warn("邮箱对应的用户不存在: email={}", email);
                    return new BusinessException("邮箱对应的用户不存在");
                });
        ...
        log.info("密码重置成功: username={}, email={}", user.getUsername(), email);
        return true;
    }
}